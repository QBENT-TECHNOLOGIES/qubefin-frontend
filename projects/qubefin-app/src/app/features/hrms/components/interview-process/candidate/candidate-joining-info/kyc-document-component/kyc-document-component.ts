...
import { candidateKycDocument, ICandidateJoiningInfo } from '../../../../../models/candidate';
...
@Component({
  selector: 'qfin-joining-kyc-document-component',
  providers: [DatePipe],
  imports: [
    CommonModule,
    MatFormFieldModule,
...
export class JoiningKycDocumentComponent {
  ...
  protected readonly kycSchema = schema<KycFormModel>((path) => {
    ...
    applyEach(path.documents, (docPath) => {
      ...
      // Verified in Candidate Verification - neither the document nor its number can change.
      readonly(docPath.documentName, {
        when: ({ valueOf }: any) => this.isVerifiedDocumentName(valueOf(docPath.documentName)),
      });
      readonly(docPath.documentNo, {
        when: ({ valueOf }: any) => this.isVerifiedDocumentName(valueOf(docPath.documentName)),
      });
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (docs.length && this.kycModel().documents.length === 0) {
        // Mandatory documents plus any document the candidate record already holds a number for.
        const mandatoryDocs = docs.filter(
          (x) => x.isMandatory || !!candidateKycDocument(this.joiningInfo(), x.name)?.documentNo,
        );
        ...
        this.kycModel.set({
          documents: mandatoryDocs.map((doc) => {
            ...
            model.documentNo = candidateKycDocument(this.joiningInfo(), doc.name)?.documentNo ?? '';
  ...
  isVerifiedDocumentName(name: string | null | undefined): boolean {
    return !!candidateKycDocument(this.joiningInfo(), name)?.isVerified;
  }
  isVerifiedDocument(index: number): boolean {
    return this.isVerifiedDocumentName(this.kycModel().documents[index]?.documentName);
  }
  ...
  onDocumentChange(index: number) {
    this.kycModel.update((state) => {
      ...
      documents[index] = {
        ...current,
        documentNo: candidateKycDocument(this.joiningInfo(), current.documentName)?.documentNo ?? '',
        validFrom: null,
        validTill: null,
      };
  ...
  removeDocument(index: number) {
    if (this.isVerifiedDocument(index)) {
      return;
    }
    this.kycModel.update((state) => ({
  ...
  onSubmit() {
    ...
    this.employeeService.updateKycInfo(this.employeeId(), formData).subscribe({
      next: (resp: any) => {
              ...
              documents: documentsArray.map(
                (doc: IEmployeeDocument) =>
                  new EmployeeDocument({
                    ...doc,
                    documentNo: this.isVerifiedDocumentName(doc.documentName)
                      ? (candidateKycDocument(this.joiningInfo(), doc.documentName)?.documentNo ??
                        doc.documentNo)
                      : doc.documentNo,
