...
import { ICandidateJoiningInfo, withCandidateContact } from '../../../../../models/candidate';
...
@Component({
  selector: 'qfin-joining-contact-component',
  imports: [
    CommonModule,
    MatFormFieldModule,
    MatInputModule,
...
export class JoiningContactComponent {
  ...
  readonly isMobileVerified = computed(() => !!this.joiningInfo()?.isMobileValidated);
  ...
  protected readonly contactSchema: Schema<IEmployeeContactInfo> = schema((path) => {
    ...
    readonly(path.mobileNo, { when: () => this.isMobileVerified() });
    ...
    stream: ({ params }) => {
      if (params.editMode && params.id !== EMPTY_UUID) {
        ...
        return this.employeeService.getContactData(params.id).pipe(
          tap((resp: any) => {
            resp = withCandidateContact(resp, this.joiningInfo());
  ...
  onSubmit() {
    ...
    if (this.isEditMode()) {
      this.employeeService.updateContactInfo(this.employeeId(), dataToSave).subscribe({
        next: (resp: any) => {
