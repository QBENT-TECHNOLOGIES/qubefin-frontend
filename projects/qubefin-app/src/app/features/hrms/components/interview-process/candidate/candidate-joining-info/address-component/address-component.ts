...
import { ICandidateJoiningInfo, withCandidateAddress } from '../../../../../models/candidate';
...
@Component({
  selector: 'qfin-joining-address-component',
  imports: [
    CommonModule,
    MatFormFieldModule,
    MatInputModule,
...
export class JoiningAddressComponent {
    ...
    stream: ({ params }) => {
      if (params.editMode && params.id !== EMPTY_UUID) {
        ...
        return this.employeeService.getAddressData(params.id).pipe(
          tap((resp: any) => {
            resp = withCandidateAddress(resp, this.joiningInfo());
  ...
  onSubmit() {
    ...
    if (this.isEditMode()) {
      this.employeeService.updateAddresslInfo(this.employeeId(), dataToSave).subscribe({
        next: (resp: any) => {
